...
class Comment(Base):
    ...
    ai_reason = Column(String(255), nullable=True, comment="AI 理由（简短）")
    ai_confidence = Column(Integer, nullable=True, comment="置信度*100，便于整型存储")
    ai_source = Column(String(50), nullable=True, comment="来源：cloud/local/cache等")
    ai_prompt_version_id = Column(Integer, nullable=True, comment="Prompt版本")
    ai_model = Column(String(100), nullable=True, comment="使用的模型")
    ai_trace = Column(Text, nullable=True, comment="决策链路JSON字符串")
